...
const userLinks = [
  ...
  { to: '/user/issue',        label: 'Issue Book',      icon: '📤' },
  { to: '/user/return',       label: 'Return Book',     icon: '📥' },
  { to: '/user/pay-fine',     label: 'Pay Fine',        icon: '💰' },
  { to: '/user/reports',      label: 'Reports',         icon: '📊' },
];
  ...
  return (
    <div className="w-60 min-h-screen bg-slate-900 text-white flex flex-col flex-shrink-0">
      <div className="p-5 border-b border-slate-700">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block font-medium ${user?.isAdmin ? 'bg-blue-600' : 'bg-green-600'}`}>
